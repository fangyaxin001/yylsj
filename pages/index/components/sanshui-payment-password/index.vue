    ...
    <sanshuiModal ref="ssModal" :mode="_parentMode" :position="_parentPosition" @change="changeModal">
      <view class="digital-keyboard-modal" :class="'digital-keyboard-modal-' + _mode">
        ...
        <block v-if="_mode !== 1 && _mode !== 4">
          ...
          <view class="pwd-tips">{{i18n['支付密码为6位数字，用户提现、设置密保、添加银行卡等操作，可保障资金安全']}}</view>
          <view class="pwd-tips pwd-tips-errot">{{ msgText }}</view>
        </view>
        </block>
        <view class="pwd-forget"><text v-if="_forget && _mode === 1" @click="forgetPwd">{{i18n['忘记支付密码']}}</text></view>
...
export default {
  ...
  computed: {
    ...
    _titleObj() {
      ...
      let arr = [
        {
          title: this.i18n['输入支付密码'],
          subTitle: this.i18n['请输入支付密码']
        },
        {
          title: this.i18n['请设置新密码,用于支付验证'],
          twoTitle: this.i18n['请再次输入,以确认密码'],
          subTitle: '',
          twoSubTitle: '',
        },
        {
          title: this.i18n['修改支付密码'],
          twoTitle: this.i18n['设置支付密码'],
          threeTitle: this.i18n['设置支付密码'],
          subTitle: this.i18n['请输入支付密码 以验证身份'],
          twoSubTitle: this.i18n['请设置新支付密码'],
          threeSubTitle: this.i18n['请再次输入新支付密码']
        },
        {
          title: this.i18n['输入支付密码'],
          subTitle: this.i18n['请输入支付密码']
        ...
        {
          title: this.i18n['重置支付密码'],
          subTitle: this.i18n['请输入支付密码'],
          twoSubTitle: this.i18n['请再次输入新支付密码']
        }
      ];
        ...
        if (this._mode === 5 || this._mode === 4 || this.current >= this._mode - 1) {
          ...
          if (this.current > 0) {
            ...
            if (this.paymentPwd !== this.paymentPwds[this.current - 1]) {
              this.showError(this.i18n['两次支付密码输入不一致']);
              this.paymentPwd = '';
