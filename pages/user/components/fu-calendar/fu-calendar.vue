<template>
  <view class="fu-calendar-wrap">
    ...
    <view class="fu-calendar-data fu-flex fu-justify-center fu-align-center fu-margin fu-height-72" v-if="changeTitle">
      ...
      <view class="fu-title fu-text-32 fu-text-333">{{ currentDate[0] }}-{{ currentDate[1]>9?currentDate[1]:'0'+currentDate[1] }}</view>
      <block v-if="changeMonth">
    ...
    <!-- 星期 end -->
    <!-- 日历 start -->
    <view class="fu-calendar fu-flex fu-justify-between fu-align-center fu-flex-wrap fu-text-666">
      ...
      >
        <view
          ...
          :style="[(item.dayActive || item.start || item.end) && !item.isNot ? dayBg : {}, item.active && !item.isNot ? activeBg : item.dayActive && item.currentSignin && !item.isNot ? currentBg:{},]"
        >
          {{ item.day }}
...
export default {
  props: {
    ...
    'active-bg-color': {
      type: String,
      default: 'linear-gradient(316deg, #F94032 0%, #F68F34 100%)'
    },
    // 选择日期中间的背景色
    'range-bg-color': {
      type: String,
  ...
  computed: {
    ...
    dayBg() {
      ...
      let url = require('./static/signin-1-circle.png');
  ...
  methods: {
    ...
    singinFun(calendar, data) {
      ...
      // let month = currentDate[1]>9?currentDate[1]:'0'+currentDate[1];
      // let day = currentDate[2]>9?currentDate[2]:'0'+currentDate[2];
    ...
    prevMonth() {
      ...
      let month = currentDate[1]>9?currentDate[1]:'0'+currentDate[1];
      let currentDay = currentDate[0]+'-'+month;
      this.$emit('switchChange',currentDay)
    },
    /**
     * @description 下月
     */
