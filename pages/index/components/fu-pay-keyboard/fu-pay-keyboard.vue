<template>
	<view v-if="show">
		...
		<!-- 金额 -->
		<view :class="['content', show ? '' : 'contenthide']" :style="[{ height: isFromPay ? 570: 370 + 'rpx' }]">
			<view class="title">
				<!-- <view class="close" @click="close()">
					<image src="./static/close.png" />
				</view> -->
				<view class="cuIcon-roundclosefill icon-close"  @tap="close()"></view>
				<view class="center"> {{isFromPay ? i18n['请输入支付密码'] : i18n['开通指纹支付'] }}</view>
				<view v-if="isFromPay" @click="handleFingerprint" class="right">{{i18n['指纹支付']}}</view>
			</view>
			<view v-if="isFromPay" class="pay-money">
				<view>{{i18n['金额']}}</view>
				<text class="money text-price fu-cred"><text>{{money}}</text></text>
			</view>
			<view v-else class="text-999">{{i18n['验证支付密码，完成身份验证']}}</view>
			<view v-if="isFromPay" class="pay-way">
				<view>{{i18n['支付方式']}}</view>
				<view class="pay-img">
					<image src="/pages/order/static/order-process/ye.png" />{{i18n['余额']}}
				</view>
			</view>
